...
// What `agent explain` actually returns, shaped like the payload read off a live
// session: every field populated, the manifest a real absolute path, and an
// `evidence` block on every rule carrying the raw screen text that must never be
// drawn. Run through the real describeDetection so the fixture cannot drift from
// what the office would actually put on a card.
export const LIVE_EXPLAIN = {
  agent: 'w1:p1',
  state: 'blocked',
  matched_rule: { id: 'tool_approval_prompt', priority: 950, region: 'bottom_non_empty_lines(5)', state: 'blocked' },
  evaluated_rules: Array.from({ length: 16 }, (_, i) => ({
    id: `rule_${i}`,
    matched: i === 3,
    evidence: {
      all_count: 1,
      any_count: 2,
      contains: ['MUST NOT BE DRAWN'],
      line_regex: 'MUST NOT BE DRAWN',
      not_count: 0,
      regex: 'MUST NOT BE DRAWN',
      region_bytes: 4096,
      region_preview: 'MUST NOT BE DRAWN: account 000000000000, /Users/somebody/secrets, sk-live-000',
    },
  })),
  ...
  remote_update_error: 'connect ETIMEDOUT 203.0.113.1:443',
  manifest_source: 'remote:/Users/somebody/.local/state/herdr/agent-detection/remote/claude.toml',
  manifest_version: '2026.09.01.1',
  cached_remote_version: '2026.08.30.2',
  warning: 'the manifest at /Users/somebody/.config/herdr/x.toml could not be parsed',
  fallback_reason: 'no_manifest',
};

// The same explanation the office would print for that payload, longest form.
export const LIVE_DETECTION = describeDetection(LIVE_EXPLAIN);
...
export const DETAILS = [
  ...
  // for, and it is drawn above the buttons that send real keystrokes.
  [
    'long explanation',
